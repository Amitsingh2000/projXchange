      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isMenuOpen && (
          <div className="lg:hidden mt-2 bg-white border-t border-gray-200 py-4 space-y-2">
            <Link to="/" className="block px-4 py-2 text-gray-700" onClick={() => setIsMenuOpen(false)}>
              Home
            </Link>
            <Link to="/projects" className="block px-4 py-2 text-gray-700" onClick={() => setIsMenuOpen(false)}>
              Projects
            </Link>
            {isAuthenticated && (
              <>
                <Link to="/upload" className="block px-4 py-2 text-gray-700" onClick={() => setIsMenuOpen(false)}>
                  Sell Project
                </Link>
                <Link to="/dashboard" className="block px-4 py-2 text-gray-700" onClick={() => setIsMenuOpen(false)}>
                  Dashboard
                </Link>
                {isAdmin && (
                  <Link to="/admin" className="block px-4 py-2 text-gray-700" onClick={() => setIsMenuOpen(false)}>
                    Admin
                  </Link>
                )}
                <button onClick={handleLogout} className="block w-full text-left px-4 py-2 text-gray-700">
                  Logout
                </button>
              </>
            )}
            {!isAuthenticated && (
              <>
                <Link to="/login" className="block px-4 py-2 text-gray-700" onClick={() => setIsMenuOpen(false)}>
                  Sign In
                </Link>
                <Link to="/signup" className="block px-4 py-2 text-blue-600 font-medium" onClick={() => setIsMenuOpen(false)}>
                  Join Now
                </Link>
              </>
            )}
